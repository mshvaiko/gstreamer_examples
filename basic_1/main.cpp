...
main (int argc, char *argv[])
{
  ...
  filter = gst_element_factory_make ("vertigotv", "vertigo-filter");
  ...
  if (gst_element_link (source, filter/* converter, sink*/) != TRUE) {
    g_printerr ("Elements could not be linked.\n");
    gst_object_unref (pipeline);
    return -1;
  }

  if (gst_element_link (filter, converter/*, sink*/) != TRUE) {
    g_printerr ("Elements could not be linked.\n");
    gst_object_unref (pipeline);
    return -1;
  }
